...
def httpEnum(ip_address, port):
    ...
    HTTPSCAN = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-email-harvest,http-methods,http-method-tamper,http-passwd,http-robots.txt -oN /home/freemandyson/Projects/pentesting_tools/scans/%s_http.nmap %s" % (port, ip_address, ip_address)
...
def httpsEnum(ip_address, port):
    ...
    print "INFO: Performing nmap web script scan for " + ip_address + ":" + port    
    HTTPSCANS = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-email-harvest,http-methods,http-method-tamper,http-passwd,http-robots.txt -oX /home/freemandyson/Projects/pentesting_tools/scans/%s_https.nmap %s" % (port, ip_address, ip_address)
...
def mssqlEnum(ip_address, port):
    ...
    MSSQLSCAN = "nmap -vv -sV -Pn -p %s --script=ms-sql-info,ms-sql-config,ms-sql-dump-hashes --script-args=mssql.instance-port=1433,smsql.username-sa,mssql.password-sa -oX /home/freemandyson/Projects/pentesting_tools/scans/%s_mssql.xml %s" % (port, ip_address, ip_address)
...
'''  
For TCP XML output: 
-oX '/home/freemandyson/Projects/pentesting_tools/scans/%s_nmap_scan_import.xml' 

For UDP XML output:
-oX '/home/freemandyson/Projects/pentesting_tools/scans/%sU_nmap_scan_import.xml'
'''
def nmapScan(ip_address):
   ip_address = ip_address.strip()
   print "INFO: Running general TCP/UDP nmap scans for " + ip_address
   serv_dict = {}
   TCPSCAN = "nmap -vv -Pn -A -sC -sS -T 4 -p- -oN '/home/freemandyson/Projects/pentesting_tools/scans/%s.nmap' -oX '/home/freemandyson/Projects/pentesting_tools/scans/nmap_xml/%s_nmap_scan_import.xml' %s"  % (ip_address, ip_address, ip_address)
   UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 -oN '/home/freemandyson/Projects/pentesting_tools/scans/%sU.nmap' -oX '/home/freemandyson/Projects/pentesting_tools/scans/nmap_xml/%sU_nmap_scan_import.xml' %s" % (ip_address, ip_address, ip_address)
